...
class AnagramTest extends FunSuite with Matchers {
  ...
  test("no matches") {
    Anagram.findAnagrams(
      "diaper",
      List("hello", "world", "zombies", "pants")
    ) should be(List())
  }
  ...
  test("detects two anagrams") {
    Anagram.findAnagrams(
      "master",
      List("stream", "pigeon", "maters")
    ) should be(List("stream", "maters"))
  }
  ...
  test("detects three anagrams") {
    ...
    ) should be(List("gallery", "regally", "largely"))
  }
  ...
  test("detects anagrams case-insensitively") {
    Anagram.findAnagrams(
      "Orchestra",
      List("cashregister", "Carthorse", "radishes")
    ) should be(List("Carthorse"))
  }
