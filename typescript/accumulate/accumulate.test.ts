...
describe("accumulate()", () => {
  it("accumulation empty", () => {
    ...
    expect(accumulate<number>([], accumulator)).toEqual([]);
  });

  it("accumulate squares", () => {
    const accumulator = (n: number): number => n * n;
    const result = accumulate<number>([1, 2, 3], accumulator);
    expect(result).toEqual([1, 4, 9]);
  });

  it("accumulate upcases", () => {
    const accumulator = (word: string): string => word.toUpperCase();
    const result = accumulate<string>("hello world".split(/\s/), accumulator);
  ...
  it("accumulate reversed strings", () => {
    const accumulator = (word: string): string =>
      word
        ...
        .reverse()
        .join("");
    const result = accumulate<string>(
  ...
  it("accumulate recursively", () => {
    const result = accumulate<string, string[]>(
      "a b c".split(/\s/),
      (char: string) =>
